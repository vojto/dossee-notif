...
function Connection(manager, options, req, socket, upgradeHead) {
  ...
  this.id = manager.createId(socket.encrypted ? socket.socket.remotePort : socket.remotePort);
  ...
  if (connection._options.debug) {
    debug = function() {
      util.error(
          (connection._socket.encrypted ? '\033[90mWSS: ' : '\033[90mWS: ') +
  ...
  if (!checkVersion(this)) {
    ...
    if (!socket.encrypted) socket.setKeepAlive(true, 0);
...
Connection.prototype.inspect = function() {
  return '<' + (this._socket.encrypted ? 'WSS' : 'WS') + ':Connection ' + this.id + '>';
};
